...
def generate_answer(query: str, evidence_blocks: list[dict], history: list | None = None) -> dict:
    context_parts = []
...
def generate_answer_stream(query: str, evidence_blocks: list[dict], history: list | None = None):
    context_parts = []
    for i, block in enumerate(evidence_blocks):
        source_type = block["source_type"]
        lines = [f"--- SOURCE {i + 1} ---"]
...
USER QUESTION: {query}"""
    ...
    yield f"data: {json.dumps({'type': 'done', 'answer': result['answer'], 'citations': result.get('citations', []), 'insufficient_evidence': result.get('insufficient_evidence', False)})}\n\n"
